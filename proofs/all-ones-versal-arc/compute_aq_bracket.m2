...
--       [y,-] : T^2_0(A) --> T^3_0(A)
--
-- for the transported rational direction y.  This uses a semi-free
...
C2classes = coker map(C2,A^(toList(16:{-3})),
  sub(transpose relationMatrix,A));
...
-- Independently recover dim T^3_0, to make clear where the bracket lands.
conormal = coker sub(presentation(I/I^2),A);
T3module = prune Ext^2(conormal,A^1);
T3dimension = numColumns basis(0,T3module);
assert(T3dimension == 24);

certificatePath =
  "proofs/all-ones-versal-arc/certificates/aq_bracket_QQ.txt";
out = certificatePath;
out << "" << flush;
out << "field=QQ" << endl;
...
out << "T3_degree_zero_dimension=24" << endl;
out << "direction=transported_rational_top_component_point" << endl;
out << "rank_ad_y_T2_to_T3=12" << endl;
out << "rank_Dq_y=15" << endl;
out << "ad_y_composed_Dq_y_zero=true" << endl;
out << "kernel_ad_y_equals_image_Dq_y=true" << endl;
out << "primary_bracket_plus_Dq_y_zero=true" << endl;
out << "primary_self_bracket_at_y_zero=true" << endl;
out << "partial_derivations_extend_by_tate_acyclicity=true" << endl;
out << "intrinsic_bracket_not_yoneda=true" << endl << flush;

print("T3 degree-zero dimension=" | toString T3dimension);
print("rank [y,-]:T2_0->T3_0=" | toString bracketRank);
print("rank Dq_y=" | toString rank DqY);
print("ker([y,-])=image(Dq_y)");
