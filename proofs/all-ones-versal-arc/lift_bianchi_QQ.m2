...
-- available homogeneous QQ base term.  This is the characteristic-zero
-- counterpart of lift_bianchi.m2.  The input contains degrees 2,...,5 only,
-- so the exact identities checked here have total degrees 3,...,6 only.

K = QQ;
B = K[t_1..t_53,MonomialOrder=>GRevLex];

inputPath = getenv "VD_BASE_QQ_EXPORT";
if inputPath === null then inputPath =
  "proofs/all-ones-versal-arc/generated/versal_base_QQ_deg2_5.txt";
certificatePath = getenv "BIANCHI_QQ_CERTIFICATE";
if certificatePath === null then certificatePath =
  "proofs/all-ones-versal-arc/certificates/bianchi_through_degree6_QQ.txt";
...
rankCheck(toList apply(0..52,index -> 1_K));
...
rankCheck(transportedGenericPoint);

out = certificatePath;
out << "" << flush;
out << "field=QQ" << endl;
out << "input=generated/versal_base_QQ_deg2_5.txt" << endl;
out << ("input_sha256="
    | "40d1d011748a0b3528464067b6c320dacef073d56304363c005230eb2ec41cc5");
out << endl;
out << "exported_base_degrees=(2,3,4,5)" << endl;
out << "complete_polynomial_base=false" << endl;
