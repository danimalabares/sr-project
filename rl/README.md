...
> **Historical search workspace.** The search described below preceded the
> characteristic-zero smoothing proof. The answer is now **yes**; see the
> repository [`README.md`](../README.md) and the exact certificates in
> [`proofs/`](../proofs/). In particular, the corrected value is
> \(\dim T^2_0=27\).
...
**Note.** This RL machine relies on 
the work stored in `old-code`.
Facts like the dimension of $T^1$,
$T^2$ and their coordinates are taken
for granted in this directory.
As of 1/8/2026, I will focus on
certifying those facts before 
continuing the work in this directory.
...
y[0] = 1                             # Choose the 0-th coordinate equal to 1
...
More generally, you can modify the
coordinates of the deformation direction
`y` by setting them manually at will,
e.g. `y[3]=-4` and so on.
...
The first exercise below shows a single
valid solution for an order-2 lift. The
actual machine will choose between different
order-2 lifts, so in this step we will compute
the whole space of order-2 lifts for
each $y$.
The function
`
...
nonzero second-order corrections, i.e. 
its second-order lift is
visibly different from its first-order
deformation.

### Exercise: compute the space of order-2 lifts for a given $y$
...
Again, it does not prove that the cubic
polynomials define a flat family over
$K[t]$.

### Exercise: compute the third-order lift space for a given $y$
