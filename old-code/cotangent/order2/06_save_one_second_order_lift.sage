...
def test_lift_to_order_2(direction, test_number):
    ...
    if lifts and test_number == 1:
        sol = A.solve_right(b)
        psi_solution = sol[:n_params]
        alpha_solution = sol[n_params:]
        ...
        print("Saved cache/one_order2_lift.sobj")
...
# Just save one nontrivial sample for now.
samples = [chosen[-1]]
N_TESTS = 1
