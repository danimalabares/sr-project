# 23_minimal_layer2_lift_search.sage
#
# Search for sparse layer-2 lifted legal corrections.
#
# Script 22 showed that only lifting m4,m5 affects the two named layer-3
# membership tests.  Here we keep m1,m2,m3 raw and search sparse q4/q5
# corrections, using only monomials from the successful q4/q5 supports.
#
# Run from:
#   code/more-lifting
#
...
OUT_FILE = "cache/23_minimal_layer2_lift_search.log"
...
def standard_degree_exps(degree):
    return [e for e in degree_exps(nvars, degree) if not monomial_in_I(e)]


STANDARD_DEG4_EXPS = standard_degree_exps(4)


def random_standard_poly_T(T, rng, degree, n_terms):
    exps = STANDARD_DEG4_EXPS if degree == 4 else standard_degree_exps(degree)
    chosen = rng.sample(exps, min(n_terms, len(exps)))
    p = T(0)
    for e in chosen:
        p += nz_rand(rng) * monomial_T_from_exp(T, e)
    return p, chosen


def lifted_repair_polys(T, rng):
    t = T.gen(0)
    q1, s1 = random_standard_poly_T(T, rng, 4, N_Q_TERMS)
    q2, s2 = random_standard_poly_T(T, rng, 4, N_Q_TERMS)
    q4, s4 = random_standard_poly_T(T, rng, 4, N_Q_TERMS)
    q5, s5 = random_standard_poly_T(T, rng, 4, N_Q_TERMS)

    q_polys = [q1, q2, T(0), q4, q5]
    repairs = [
        monomial_T_from_exp(T, LAYER1_EXPS[0]) + t * q1,
        monomial_T_from_exp(T, LAYER1_EXPS[1]) + t * q2,
        monomial_T_from_exp(T, LAYER1_EXPS[2]),  # high degree witness: q_3 = 0
        monomial_T_from_exp(T, LAYER2_EXPS[0]) + t * q4,
        monomial_T_from_exp(T, LAYER2_EXPS[1]) + t * q5,
    ]
    supports = [s1, s2, [], s4, s5]
    return repairs, supports, q_polys


def raw_legal_repair_polys(T):
    return [
        monomial_T_from_exp(T, LAYER1_EXPS[0]),
        monomial_T_from_exp(T, LAYER1_EXPS[1]),
        monomial_T_from_exp(T, LAYER1_EXPS[2]),
        monomial_T_from_exp(T, LAYER2_EXPS[0]),
        monomial_T_from_exp(T, LAYER2_EXPS[1]),
    ]
...
def nonzero_t0_remainders_mod_I(T, S, I_S, ideal_T):
    gb_I = I_S.groebner_basis()
    out = []
    for g in ideal_T.gens():
        g0 = specialize_t0_poly(T, S, g)
        r = S(g0).reduce(gb_I)
        if r != 0:
            out.append(r)
    return out
...
def make_repair(T, base_exp, q_poly=None):
    t = T.gen(0)
    h = monomial_T_from_exp(T, base_exp)
    if q_poly is not None:
        h += t * q_poly
    return h


def first_colon_diagnostic(T, t, S, Jbase, I_S, repairs, u1, u2):
    Jtest = T.ideal(list(Jbase.gens()) + repairs)
    raw0 = specialize_t0_ideal(T, t, S, Jtest)
    raw0_eq_I = ideal_eq(I_S, raw0)
    gb_J = Jtest.groebner_basis()
    tu1_rem = T(t * u1).reduce(gb_J)
    tu2_rem = T(t * u2).reduce(gb_J)

    colon = Jtest.quotient(T.ideal(t))
    colon_eq = (colon == Jtest)
    gb_I = I_S.groebner_basis()
    witnesses = []
    illegal = []
    legal = []
    for g in colon.gens():
        ...
        witnesses.append(r)
        r0 = specialize_t0_poly(T, S, r)
        r0_mod_I = S(r0).reduce(gb_I)
        if r0_mod_I == 0:
            legal.append(r)
        else:
            illegal.append((r, r0_mod_I))
    return {
        "raw0_eq_I": raw0_eq_I,
        "tu1_in": tu1_rem == 0,
        "tu2_in": tu2_rem == 0,
        ...
        "first_illegal": illegal[:MAX_WITNESSES],
    }


def diagnostic_score(result):
    return (
        int(result["tu1_in"]) + int(result["tu2_in"]),
        result["n_illegal"],
        result["n_witnesses"],
    )


def run_named_ansatz(T, t, S, J, I_S, u1, u2, name, repairs, print_illegal=False):
    result = first_colon_diagnostic(T, t, S, J, I_S, repairs, u1, u2)
    pr("  %-28s raw0 %s tu1 %s tu2 %s witnesses %4d legal %4d illegal %4d" % (
        name,
        result["raw0_eq_I"],
        result["tu1_in"],
        result["tu2_in"],
    ...
    if print_illegal:
        for i, (r, r0) in enumerate(result["first_illegal"][:3]):
            pr("      illegal[%d] t0 mod I = %s" % (i, r0))
    return result


def minimal_layer2_search(name, gen_vectors):
    pr(name)
    pr("  building base family ideal")
    T, t, S, J, I_S, f_S = build_family_ideal(gen_vectors)
    pr("  generator nnz = %s" % {k: nnz(v) for k, v in gen_vectors.items()})

    u1 = monomial_T_from_exp(T, LAYER3_EXPS[0])
    u2 = monomial_T_from_exp(T, LAYER3_EXPS[1])

    repair_rng = random.Random(builtins.int(20261620))
    repairs, supports, q_polys = lifted_repair_polys(T, repair_rng)

    raw_repairs = [
        make_repair(T, LAYER1_EXPS[0]),
        make_repair(T, LAYER1_EXPS[1]),
        make_repair(T, LAYER1_EXPS[2]),
        make_repair(T, LAYER2_EXPS[0]),
        make_repair(T, LAYER2_EXPS[1]),
    ]

    pr("")
    pr("Baseline")
    pr("--------")
    baseline = run_named_ansatz(T, t, S, J, I_S, u1, u2, "raw_all", raw_repairs, True)

    q4_terms = [(e, monomial_T_from_exp(T, e)) for e in supports[3]]
    q5_terms = [(e, monomial_T_from_exp(T, e)) for e in supports[4]]

    pr("")
    pr("Singleton q4 tests: h4=m4+t*q4, h5 raw")
    pr("---------------------------------------")
    q4_rows = []
    for idx, (e, mon) in enumerate(q4_terms):
        repairs_i = raw_repairs[:3] + [make_repair(T, LAYER2_EXPS[0], mon), raw_repairs[4]]
        result = run_named_ansatz(T, t, S, J, I_S, u1, u2, "q4_%02d %s" % (idx, e), repairs_i)
        q4_rows.append((diagnostic_score(result), idx, e, mon, result))

    pr("")
    pr("Singleton q5 tests: h4 raw, h5=m5+t*q5")
    pr("---------------------------------------")
    q5_rows = []
    for idx, (e, mon) in enumerate(q5_terms):
        repairs_i = raw_repairs[:4] + [make_repair(T, LAYER2_EXPS[1], mon)]
        result = run_named_ansatz(T, t, S, J, I_S, u1, u2, "q5_%02d %s" % (idx, e), repairs_i)
        q5_rows.append((diagnostic_score(result), idx, e, mon, result))

    q4_rows.sort(key=lambda row: row[0])
    q5_rows.sort(key=lambda row: row[0])
    best_q4 = q4_rows[:4]
    best_q5 = q5_rows[:4]

    pr("")
    pr("Best singleton q4 rows")
    pr("----------------------")
    for score, idx, e, mon, result in best_q4:
        pr("q4_%02d %s score %s witnesses %d illegal %d tu1 %s tu2 %s" % (
            idx, e, score, result["n_witnesses"], result["n_illegal"],
            result["tu1_in"], result["tu2_in"],
        ))

    pr("")
    pr("Best singleton q5 rows")
    pr("----------------------")
    for score, idx, e, mon, result in best_q5:
        pr("q5_%02d %s score %s witnesses %d illegal %d tu1 %s tu2 %s" % (
            idx, e, score, result["n_witnesses"], result["n_illegal"],
            result["tu1_in"], result["tu2_in"],
        ))
    ...
    pr("---------------")
    pair_rows = []
    for score4, idx4, e4, mon4, _ in best_q4:
        for score5, idx5, e5, mon5, _ in best_q5:
            repairs_i = raw_repairs[:3] + [
                make_repair(T, LAYER2_EXPS[0], mon4),
                make_repair(T, LAYER2_EXPS[1], mon5),
            ]
            label = "q4_%02d + q5_%02d" % (idx4, idx5)
            result = run_named_ansatz(T, t, S, J, I_S, u1, u2, label, repairs_i)
            pair_rows.append((diagnostic_score(result), idx4, idx5, e4, e5, result))

    pair_rows.sort(key=lambda row: row[0])
    pr("")
    pr("Best pairs")
    pr("----------")
    for score, idx4, idx5, e4, e5, result in pair_rows[:8]:
        pr("q4_%02d %s + q5_%02d %s score %s witnesses %d illegal %d tu1 %s tu2 %s" % (
            idx4, e4, idx5, e5, score, result["n_witnesses"],
            result["n_illegal"], result["tu1_in"], result["tu2_in"],
        ))


pr("Minimal layer-2 lift search")
pr("---------------------------")
pr("SPARSE_SUPPORT = %s" % SPARSE_SUPPORT)
pr("MAX_ORDER_FOR_CUBIC_SOLVE = %d" % MAX_ORDER)
pr("N_Q_TERMS = %d" % N_Q_TERMS)
pr("standard degree-4 monomials modulo I = %d" % len(STANDARD_DEG4_EXPS))
pr("")

pr("Preparing saved sparse branch")
pr("-----------------------------")
saved = saved_branch_state()
minimal_layer2_search("saved_order30_branch", saved["gen_vectors"])
