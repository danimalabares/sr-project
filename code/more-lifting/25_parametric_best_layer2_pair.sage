# 25_parametric_best_layer2_pair.sage
#
# Two-parameter test for the best sparse layer-2 correction.
#
# Script 23 found a much smaller correction.  Here we test
#
#   h4 = m4 + t*a*x0*x1*x3*x7
#   h5 = m5 + t*b*x1*x3^2*x4
#
# with m1,m2,m3 raw, over a deterministic finite-field grid.
#
# Run from:
#   code/more-lifting
#
...
OUT_FILE = "cache/25_parametric_best_layer2_pair.log"
...
PARAM_VALUES = [K(1), K(2), -K(1), -K(2), K(4457), -K(4457), K(2333), -K(2333)]


def best_layer2_repairs(T, a, b):
    q4 = monomial_T_from_exp(T, (1, 1, 0, 1, 0, 0, 0, 1))  # x0*x1*x3*x7
    q5 = monomial_T_from_exp(T, (0, 1, 0, 2, 1, 0, 0, 0))  # x1*x3^2*x4
    repairs = [
        monomial_T_from_exp(T, LAYER1_EXPS[0]),
        monomial_T_from_exp(T, LAYER1_EXPS[1]),
        monomial_T_from_exp(T, LAYER1_EXPS[2]),
        monomial_T_from_exp(T, LAYER2_EXPS[0]) + T.gen(0) * K(a) * q4,
        monomial_T_from_exp(T, LAYER2_EXPS[1]) + T.gen(0) * K(b) * q5,
    ]
    return repairs, q4, q5


def evaluate_pair(T, t, S, J, I_S, u1, u2, a, b):
    repairs, q4, q5 = best_layer2_repairs(T, a, b)
    Jprime = T.ideal(list(J.gens()) + repairs)
    raw0 = specialize_t0_ideal(T, t, S, Jprime)
    raw0_eq_I = ideal_eq(I_S, raw0)
    gb_Jp = Jprime.groebner_basis()
    tu1_rem = T(t * u1).reduce(gb_Jp)
    tu2_rem = T(t * u2).reduce(gb_Jp)
    tu1_in = (tu1_rem == 0)
    tu2_in = (tu2_rem == 0)
    ...
    saturated = (Ksat.quotient(T.ideal(t)) == Ksat)
    K0 = specialize_t0_ideal(T, t, S, Ksat)
    K0_eq_I = ideal_eq(I_S, K0)
    extras = nonzero_t0_remainders_mod_I(T, S, I_S, Ksat)
    ...
    return {
        ...
        "K0_eq_I": K0_eq_I,
        "n_extras": len(extras),
        "extras": extras[:MAX_WITNESSES],
    }


def parametric_best_layer2_pair(name, gen_vectors):
    pr(name)
    ...
    pr("")
    pr("Two-parameter sparse layer-2 correction")
    pr("---------------------------------------")
    _, q4, q5 = best_layer2_repairs(T, K(1), K(1))
    pr("q4 = %s" % q4)
    pr("q5 = %s" % q5)
    pr("parameter values = %s" % [int(v) for v in PARAM_VALUES])
    ...
    for a in PARAM_VALUES:
        for b in PARAM_VALUES:
            result = evaluate_pair(T, t, S, J, I_S, u1, u2, a, b)
            results.append(result)
            pr("a=%5s b=%5s | tu1 %s tu2 %s | layers %s | sat %s | K0==I %s | extras %d" % (
                int(K(a)), int(K(b)), result["tu1_in"], result["tu2_in"],
                result["layer_counts"], result["saturated"], result["K0_eq_I"],
                result["n_extras"],
            ))

    hits = [r for r in results if r["K0_eq_I"]]
    best = sorted(results, key=lambda r: (
        int(r["tu1_in"]) + int(r["tu2_in"]),
        r["n_extras"],
        sum(r["layer_counts"]),
        len(r["layer_counts"]),
    ))
    ...
    pr("Summary")
    pr("-------")
    pr("grid points tested = %d" % len(results))
    pr("K0==I hits = %d" % len(hits))
    pr("best rows:")
    for r in best[:8]:
        pr("a=%s b=%s tu1=%s tu2=%s layers=%s extras=%d saturated=%s K0==I=%s" % (
            int(r["a"]), int(r["b"]), r["tu1_in"], r["tu2_in"],
            r["layer_counts"], r["n_extras"], r["saturated"], r["K0_eq_I"],
        ))
        for i, extra in enumerate(r["extras"][:4]):
            pr("  extra[%d] = %s" % (i, extra))


pr("Parametric best sparse layer-2 pair")
pr("-----------------------------------")
...
parametric_best_layer2_pair("saved_order30_branch", saved["gen_vectors"])
