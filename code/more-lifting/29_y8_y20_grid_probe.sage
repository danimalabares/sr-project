...
#
# Probe the two-coordinate y8,y20 branch.
#
# Script 28 found a zero-extra, t-saturated family at y20=37, y8=1.
# This script keeps the same intrinsic algebra setup and probes whether that
# is stable across a finite-field coefficient grid in the two variables y8,y20.
#
# Run from:
#   code/more-lifting
#
...
Y20_GRID = [1, 2, 3, 5, 7, 11, 37, 32002]
Y8_GRID = [1, 2, 3, 5, 7, 11, 32002]
PROMISING_EXTRA_THRESHOLD = 2
PROMISING_MAX_ORDER = 6
MAX_PROMISING_RERUNS = 4
SEED_BASE = 20260629
...
Y8_Y20_SUPPORT = sorted(SPARSE_SUPPORT + [8, 20])


def y8_y20_point(y8_coeff, y20_coeff):
    y = y20_line_point(y20_coeff)
    y[8] = K(y8_coeff)
    if not is_quadratic_solution(y):
        return None
    return y
...
pr("y8,y20 coefficient-grid probe")
pr("-----------------------------")
pr("SPARSE_SUPPORT = %s" % SPARSE_SUPPORT)
pr("Y8_Y20_SUPPORT = %s" % Y8_Y20_SUPPORT)
pr("Y8_GRID = %s" % Y8_GRID)
pr("Y20_GRID = %s" % Y20_GRID)
...
pr("MAX_PROMISING_RERUNS = %d" % MAX_PROMISING_RERUNS)
pr("")
...
pr("Quadratic filter")
pr("----------------")
for y20_coeff in Y20_GRID:
    for y8_coeff in Y8_GRID:
        y = y8_y20_point(y8_coeff, y20_coeff)
        q_ok = (y is not None)
        tested.append((y8_coeff, y20_coeff, q_ok))
        if q_ok:
            quadratic_candidates.append((y8_coeff, y20_coeff, y))
            pr("quadratic survivor: y8=%s y20=%s support=%s" % (
                K(y8_coeff), K(y20_coeff), support(y)
            ))
...
pr("---------------------")
for y8_coeff, y20_coeff, y in quadratic_candidates:
    name = "y8_%s_y20_%s" % (y8_coeff, y20_coeff)
    ...
    pr("%s" % name)
    pr("  y8 = %s, y20 = %s" % (K(y8_coeff), K(y20_coeff)))
...
pr("-----------------")
pr("reached MAX_ORDER = %d / %d quadratic candidates" % (
    len(survivors), len(quadratic_candidates)
))
for row in survivors:
    pr("%s y8=%s y20=%s support_size=%d support=%s" % (
        row["name"], row["y8"], row["y20"], row["support_size"], row["support"]
    ))
pr("")
...
for row, result in flat_results:
    pr("%s: y8=%s y20=%s, support_size %d, extras %s, J:(t)==J %s, K0==I %s, sat_steps %d, saturated %s" % (
        row["name"], row["y8"], row["y20"], row["support_size"],
        result["extra_count"], result["colon_eq"], result["K0_eq_I"],
        result["sat_steps"], result["t_saturated"]
    ))
...
pr("----------------")
if promising:
    for row, result in promising[:MAX_PROMISING_RERUNS]:
        y = y8_y20_point(row["y8"], row["y20"])
...
if flat_results:
    ...
    zero_hits = [row for row, result in flat_results if result["extra_count"] == 0]
    pr("zero-extra hits = %d" % len(zero_hits))
    if zero_hits:
        pr("zero-extra coefficient pairs = %s" % [
            (int(row["y8"]), int(row["y20"])) for row in zero_hits
        ])
    if best_count is not None and best_count == 0:
        pr("The y8,y20 branch has coefficient-grid hits with no visible flatness defect.")
    else:
        pr("No tested y8,y20 pair reached zero visible extras.")
else:
    pr("No y8,y20 candidate reached order 4.")
