# 06_order4_locus.sage
#
# Sweep the 27 irreducible product components of the quadratic obstruction
# cone and test a generic first-order direction on each for lifting through
# order 4 over GF(32003).
#
# Run from:
#   code/more-lifting
#
...
OUT_FILE = "cache/06_order4_locus.log"
...
def component_dimension(component):
    return sum(COMPONENT_DIM[i] for i in component) + len(FREE_BLOCK)


components = [(a, b, c) for a in range(3) for b in range(3) for c in range(3)]

pr("Order-4 component sweep")
pr("-----------------------")
pr("TARGET_ORDER = %d" % TARGET_ORDER)
pr("N_SAMPLES_PER_COMPONENT = %d" % N_SAMPLES_PER_COMPONENT)
...
for comp_idx, component in enumerate(components):
    dim = component_dimension(component)
    pr("Component %s (dimension %d)" % (component, dim))
    pr("  -------------------------")

    for sample_idx in range(N_SAMPLES_PER_COMPONENT):
        seed = SEED_BASE + 1000 * comp_idx + sample_idx
        y = make_component_point(component, seed)
        direction = y_to_direction(y)

        pr("  sample %d seed %d" % (sample_idx + 1, seed))
        pr("    y support size = %d, direction nnz = %d, quadratic_ok = %s" % (
            len(support(y)), nnz(direction), is_quadratic_solution(y)
        ))
        ...
        results.append({
            ...
            "order2": order_infos[2]["success"],
            "order3": order_infos.get(3, {}).get("success", False),
            "order4": order_infos.get(4, {}).get("success", False),
        })
...
for dim in sorted(by_dim):
    items = by_dim[dim]
    survivors = [item for item in items if item["order4"]]
    failures = [item for item in items if not item["order4"]]
    pr("dimension %d: order-4 survivors = %d / %d" % (dim, len(survivors), len(items)))
    if survivors:
        pr("  survivors: %s" % [item["component"] for item in survivors])
    if failures:
        max_reached_counts = Counter(item["max_order_reached"] for item in failures)
        pr("  failure max-order histogram: %s" % dict(sorted(max_reached_counts.items())))

pr("")
pr("All component results:")
for item in results:
    pr("  component %s dim %d seed %d: order2=%s order3=%s order4=%s max=%d" % (
        item["component"], item["dimension"], item["seed"],
        item["order2"], item["order3"], item["order4"], item["max_order_reached"]
    ))

order4_survivors = [item for item in results if item["order4"]]
pr("")
pr("Total order-4 survivors = %d / %d" % (len(order4_survivors), len(results)))
if order4_survivors:
    pr("Surviving components: %s" % [item["component"] for item in order4_survivors])
else:
    pr("No sampled generic component point lifted to order 4.")
