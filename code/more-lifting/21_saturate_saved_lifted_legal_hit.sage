...
#
# Saturate the saved successful lifted-legal hit from script 20.
#
# Script 20 found that the first random lifted-legal trial already preserves
# the raw special fiber and makes t*u1,t*u2 stop being consequences.  Here we
# reconstruct exactly that deterministic trial, print the q_i, and test full
# t-saturation.
#
# Run from:
#   code/more-lifting
...
OUT_FILE = "cache/21_saturate_saved_lifted_legal_hit.log"
...
def saturate_saved_hit(name, gen_vectors):
    pr(name)
    ...
    pr("")
    pr("Saved lifted-legal hit")
    pr("----------------------")
    for i, q in enumerate(q_polys, 1):
        pr("q_%d = %s" % (i, q))
        if supports[i - 1]:
            pr("  support_%d = %s" % (i, supports[i - 1]))
        else:
            pr("  support_%d = []" % i)
...
pr("Saturating saved lifted-legal hit")
pr("---------------------------------")
...
saturate_saved_hit("saved_order30_branch", saved["gen_vectors"])
