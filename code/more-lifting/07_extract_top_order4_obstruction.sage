# 07_extract_top_order4_obstruction.sage
#
# Extract a concrete order-4 compatibility witness at a generic point of
# the top quadratic component (0,0,0), then compare this with the saved
# sparse formal branch that lifts to order 30.
#
# This does not yet produce the global polynomial equation on the top
# component.  It produces the linear-algebra certificate at one sampled
# point:
#
#   lambda * A = 0,  lambda * b = 1,
#
# for the failed order-4 system A x = b.  The next step is to interpolate
# such compatibility data over the top-component parameters.
#
# Run from:
#   code/more-lifting
#
...
OUT_FILE = "cache/07_extract_top_order4_obstruction.log"

PRIME = 32003
K = GF(PRIME)

TARGET_ORDER = 4
TOP_SAMPLE_SEED = 20260629
...
pr("Top-component order-4 obstruction witness")
pr("------------------------------------------")
pr("TOP_SAMPLE_SEED = %d" % TOP_SAMPLE_SEED)
pr("")

top_y = make_component_point((0, 0, 0), TOP_SAMPLE_SEED)
top_direction = y_to_direction(top_y)

pr("Generic top-component sample")
pr("----------------------------")
pr("quadratic_ok = %s" % is_quadratic_solution(top_y))
pr("y support size = %d" % len(support(top_y)))
pr("direction nnz = %d" % nnz(top_direction))

info2, top_state = solve_order_2(top_direction)
pr("order 2: lifts = %s, order1_ok = %s, rank_A = %d, rank_B = %d" % (
    info2["success"], info2["order1_ok"], info2["rank_A"], info2["rank_B"]
))

if top_state is None:
    raise RuntimeError("top sample unexpectedly failed at order 2")

info3 = extend_one_order(top_state, 3)
pr("order 3: lifts = %s, rank_A = %d, rank_B = %d, equations = %d" % (
    info3["success"], info3["rank_A"], info3["rank_B"], info3["n_equations"]
))

if not info3["success"]:
    raise RuntimeError("top sample unexpectedly failed at order 3")

top_terms = {k: direction_to_terms(v) for k, v in top_state["gen_vectors"].items()}
top_A4, top_b4 = build_order_system(4, top_terms, top_state["syz_vectors"])
rank_A4, rank_B4, lifts4 = rank_test(top_A4, top_b4)
pr("order 4: lifts = %s, rank_A = %d, rank_B = %d, equations = %d" % (
    lifts4, rank_A4, rank_B4, top_A4.nrows()
))

if lifts4:
    raise RuntimeError("top sample unexpectedly lifted to order 4")

pr("")
pr("Extracting left-kernel compatibility witness")
pr("--------------------------------------------")
lam, left_residual, witness_value = extract_left_obstruction_witness(top_A4, top_b4)
pr("lambda length = %d" % len(lam))
pr("lambda nnz = %d" % nnz(lam))
pr("A^T lambda is zero = %s" % (left_residual == 0))
pr("b.lambda = %d" % ff_int(witness_value))
pr("first 40 nonzero lambda entries = %s" % (
    [(i, ff_int(c)) for i, c in enumerate(lam) if c != 0][:40]
))

pr("")
pr("Saved sparse formal branch")
pr("--------------------------")
saved = saved_branch_state()
saved_direction = saved["gen_vectors"][1]
saved_y = project_to_y(saved_direction)
pr("source status = %s" % saved["source_status"])
pr("source max_order_reached = %s" % saved["max_order_reached"])
pr("projected y support = %s" % support(saved_y))
pr("projected y nonzero coefficients = %s" % (
    [(i, ff_int(c)) for i, c in enumerate(saved_y) if c != 0]
))
pr("quadratic_ok = %s" % is_quadratic_solution(saved_y))
pr("direction nnz = %d" % nnz(saved_direction))
...
pr("saved order 4: lifts = %s, rank_A = %d, rank_B = %d, equations = %d" % (
    saved_lifts4, saved_rank_A4, saved_rank_B4, saved_A4.nrows()
))

pr("")
pr("Conclusion")
pr("----------")
pr("The generic top-component point has an explicit left-kernel obstruction witness.")
pr("The saved sparse branch satisfies the order-4 rank test using its saved lower-order corrections.")
pr("Next: interpolate the compatibility condition over top-component parameters.")
