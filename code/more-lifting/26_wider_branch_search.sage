# 26_wider_branch_search.sage
#
# Search for wider first-order deformation directions near the known
# 7-coordinate sparse branch.
#
# The previous scripts showed that the exact sparse slice is formally good but
# flatness-defective for the tested family ideals.  This script opens the
# search by moving inside the top quadratic component and deliberately turning
# on extra A/B/C/free coordinates around the sparse support.  Survivors are
# tested order-by-order and then checked by t-saturation.
#
# Run from:
#   code/more-lifting
#
...
OUT_FILE = "cache/26_wider_branch_search.log"
...
def sparse_like_top_point(toggles):
    toggles = set(toggles)
    y = [K(0)] * 53

    a = K(2)
    b = K(11) if "A_b" in toggles else K(0)
    u0 = K(3)
    u1 = K(5)
    u2 = K(7) if "A_u2" in toggles else K(0)
    v0 = K(13) if "A_v0" in toggles else K(0)
    v1 = K(17) if "A_v1" in toggles else K(0)
    v2 = K(19) if "A_v2" in toggles else K(0)
    ...
    c = K(23) if "B_c" in toggles else K(0)
    d = K(29)
    w0 = K(31)
    w1 = K(37) if "B_w1" in toggles else K(0)
    w2 = K(41) if "B_tail" in toggles else K(0)
    w3 = K(43) if "B_tail" in toggles else K(0)
    w4 = K(47) if "B_tail" in toggles else K(0)
    w5 = K(53) if "B_tail" in toggles else K(0)
    put_values(y, [1, 4, 22, 26, 27, 29],
    ...
    e = K(59)
    f = K(61) if "C_f" in toggles else K(0)
    p0 = K(67) if "C_p0" in toggles else K(0)
    p1 = K(71) if "C_p1" in toggles else K(0)
    p2 = K(73) if "C_tail" in toggles else K(0)
    p3 = K(79) if "C_tail" in toggles else K(0)
    q0 = K(83) if "C_q0" in toggles else K(0)
    q1 = K(89)
    ...
    if not is_quadratic_solution(y):
        bad = [i for i, value in enumerate(eval_quadrics(y)) if value != 0]
        raise RuntimeError("wider candidate failed quadratic equations: %s" % bad)
    return y
...
def candidate_specs():
    specs = [
        ...
        ("A_v0_C_q0", ["A_v0", "C_q0"]),
        ("B_c_C_p0", ["B_c", "C_p0"]),
        ("A_b_B_w1_C_q0", ["A_b", "B_w1", "C_q0"]),
        ("A_u2_A_v0_B_tail", ["A_u2", "A_v0", "B_tail"]),
        ("C_q0_C_p0_C_f", ["C_q0", "C_p0", "C_f"]),
        ("mixed_abc_free", ["A_u2", "A_v0", "B_w1", "C_q0", "F5"]),
        ("wider_top_patch", ["A_b", "A_v0", "B_c", "B_w1", "C_q0", "C_p0", "F5"]),
    ]
    return specs


pr("Wider branch search")
pr("-------------------")
pr("SPARSE_SUPPORT = %s" % SPARSE_SUPPORT)
pr("MAX_ORDER = %d" % MAX_ORDER)
pr("MAX_FLATNESS_CANDIDATES = %d" % MAX_FLATNESS_CANDIDATES)
...
pr("Formal lifting tests")
pr("--------------------")
for name, toggles in candidate_specs():
    y = sparse_like_top_point(toggles)
...
pr("Survivors")
pr("---------")
pr("reached MAX_ORDER = %d / %d" % (len(survivors), len(tested)))
for row in survivors:
    pr("%s support_size=%d support=%s" % (
        row["name"], row["support_size"], row["support"]
    ))
pr("")

pr("Flatness diagnostics for first survivors")
pr("----------------------------------------")
flat_results = []
for row in survivors[:MAX_FLATNESS_CANDIDATES]:
...
for row, result in flat_results:
    pr("%s: support_size %d, J:(t)==J %s, K0==I %s, sat_steps %d" % (
        row["name"], row["support_size"], result["colon_eq"],
        result["K0_eq_I"], result["sat_steps"]
    ))

if not flat_results:
    pr("No wider candidate reached MAX_ORDER; next search should use a different")
    pr("quadratic component or solve directly for higher-order-compatible loci.")
