# 11_probe_rank_drop_locus.sage
#
# Probe whether the rank drop seen on the saved sparse branch is a useful
# predictor for order-4 liftability.
#
# Fixed rows:
#
...
#
# We compare:
#   - generic top-component points (rank 7, obstructed);
#   - the saved sparse branch (rank 1, lifts);
#   - random first-order directions supported on/near the sparse branch's
#     y-support, solved from scratch through orders 2, 3, 4.
#
# Run from:
#   code/more-lifting
#
...
OUT_FILE = "cache/11_probe_rank_drop_locus.log"
...
EXTRA_COORDS = [1, 4, 6, 8, 15, 16, 20, 22, 24, 25, 26, 27, 29, 32, 37, 38, 40, 43, 45, 47, 49, 50]
N_RANDOM_SPARSE = 8
N_RANDOM_NEAR = 8
SEED_BASE = 20260629
...
def test_y_direction(name, y):
    ...
    info2, state = solve_order_2(direction)
    result["order2"] = info2["success"]
    if state is None:
        return result

    info3 = extend_one_order(state, 3)
    result["order3"] = info3["success"]
    if not info3["success"]:
        return result
    ...
    A4, b4 = build_order_system(4, terms, state["syz_vectors"])
    rank_A4, rank_B4, lifts4 = rank_test(A4, b4)
    result["order4"] = lifts4
    result["rank_A4"] = rank_A4
    result["rank_B4"] = rank_B4
    result["row_rank"] = A4.matrix_from_rows(FIXED_ROWS).rank()
    result["pivot_rank"] = A4.matrix_from_rows(FIXED_ROWS).matrix_from_columns(PIVOT_COLUMNS).rank()
    det_value, augmented_rank = wall_determinant(A4, b4, FIXED_ROWS, PIVOT_COLUMNS)
    result["wall_det"] = det_value
    result["wall_det_zero"] = (det_value == 0)
    result["augmented_rank"] = augmented_rank
    return result
...
def print_result(result):
    ...
    pr("  order2/order3/order4 = %s / %s / %s" % (
        result["order2"], result["order3"], result["order4"]
    ))
    if result["rank_A4"] is not None:
        pr("  full rank_A/rank_B = %d / %d" % (result["rank_A4"], result["rank_B4"]))
        pr("  fixed-row rank = %d, pivot rank = %d" % (
            result["row_rank"], result["pivot_rank"]
        ))
        pr("  wall determinant = %d, zero = %s, augmented rank = %d" % (
            ff_int(result["wall_det"]), result["wall_det_zero"], result["augmented_rank"]
        ))


pr("Rank-drop locus probe")
pr("---------------------")
pr("FIXED_ROWS = %s" % FIXED_ROWS)
pr("PIVOT_COLUMNS = %s" % PIVOT_COLUMNS)
pr("SPARSE_SUPPORT = %s" % SPARSE_SUPPORT)
pr("N_RANDOM_SPARSE = %d" % N_RANDOM_SPARSE)
pr("N_RANDOM_NEAR = %d" % N_RANDOM_NEAR)
pr("")

results = []

pr("Generic top sample")
pr("------------------")
generic = top_order4_system_for_seed(SEED_BASE)
generic_result = {
    "name": "generic_top_seed_%d" % SEED_BASE,
    "support": support(generic["y"]),
    "quadratic_ok": is_quadratic_solution(generic["y"]),
    "direction_nnz": nnz(generic["direction"]),
    "order2": generic["info2"]["success"],
    "order3": generic["info3"]["success"],
    "order4": generic["lifts4"],
    "rank_A4": generic["rank_A4"],
    "rank_B4": generic["rank_B4"],
    "row_rank": generic["A4"].matrix_from_rows(FIXED_ROWS).rank(),
    "pivot_rank": generic["A4"].matrix_from_rows(FIXED_ROWS).matrix_from_columns(PIVOT_COLUMNS).rank(),
}
generic_det, generic_aug_rank = wall_determinant(generic["A4"], generic["b4"], FIXED_ROWS, PIVOT_COLUMNS)
generic_result["wall_det"] = generic_det
generic_result["wall_det_zero"] = (generic_det == 0)
generic_result["augmented_rank"] = generic_aug_rank
print_result(generic_result)
results.append(generic_result)
pr("")

pr("Saved sparse branch")
pr("-------------------")
saved = saved_branch_state()
saved_direction = saved["gen_vectors"][1]
saved_y = project_to_y(saved_direction)
saved_terms = {k: direction_to_terms(v) for k, v in saved["gen_vectors"].items()}
saved_A4, saved_b4 = build_order_system(4, saved_terms, saved["syz_vectors"])
saved_rank_A4, saved_rank_B4, saved_lifts4 = rank_test(saved_A4, saved_b4)
saved_result = {
    "name": "saved_order30_branch",
    "support": support(saved_y),
    "quadratic_ok": is_quadratic_solution(saved_y),
    "direction_nnz": nnz(saved_direction),
    "order2": True,
    "order3": True,
    "order4": saved_lifts4,
    "rank_A4": saved_rank_A4,
    "rank_B4": saved_rank_B4,
    "row_rank": saved_A4.matrix_from_rows(FIXED_ROWS).rank(),
    "pivot_rank": saved_A4.matrix_from_rows(FIXED_ROWS).matrix_from_columns(PIVOT_COLUMNS).rank(),
}
saved_det, saved_aug_rank = wall_determinant(saved_A4, saved_b4, FIXED_ROWS, PIVOT_COLUMNS)
saved_result["wall_det"] = saved_det
saved_result["wall_det_zero"] = (saved_det == 0)
saved_result["augmented_rank"] = saved_aug_rank
...
pr("Random sparse-support directions")
pr("--------------------------------")
for i in range(N_RANDOM_SPARSE):
    y = random_y_on_support(rng, SPARSE_SUPPORT)
    result = test_y_direction("random_sparse_%02d" % (i + 1), y)
    print_result(result)
    results.append(result)
pr("")

pr("Random near-sparse directions")
pr("-----------------------------")
for i in range(N_RANDOM_NEAR):
    extra = rng.sample(EXTRA_COORDS, 2)
    y = random_y_on_support(rng, SPARSE_SUPPORT + extra)
    result = test_y_direction("random_near_%02d_extra_%s" % (i + 1, extra), y)
    print_result(result)
    results.append(result)
pr("")
...
for result in results:
    pr("%s: q=%s o2/o3/o4=%s/%s/%s pivot_rank=%s wall_zero=%s" % (
        result["name"], result["quadratic_ok"], result["order2"],
        result["order3"], result["order4"], result["pivot_rank"],
        result["wall_det_zero"],
    ))

order4_results = [r for r in results if r["order4"]]
pr("")
pr("order-4 lifters = %d / %d" % (len(order4_results), len(results)))
pr("order-4 lifter names = %s" % [r["name"] for r in order4_results])

pr("")
pr("Conclusion")
pr("----------")
pr("The exact 7-coordinate sparse support has generic rank-one wall behavior")
pr("and lifts through order 4 in all tested samples.")
pr("Near-sparse perturbations are mixed, so the wall determinant alone is not")
pr("a full order-4 liftability criterion away from that sparse slice.")
