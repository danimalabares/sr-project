# 12_sparse_slice_higher_order.sage
#
# Test whether generic points on the 7-coordinate sparse slice lift beyond
# order 4.
#
# Fixed rows:
#
...
#
# The saved branch on this support lifts formally to order 30.  The question
# here is whether that is generic on the whole sparse slice or whether higher
# orders cut the 7-dimensional slice further.
#
# Run from:
#   code/more-lifting
#
...
OUT_FILE = "cache/12_sparse_slice_higher_order.log"
...
pr("Sparse-slice higher-order probe")
pr("--------------------------------")
...
saved_result = iterate_y_direction("saved_branch_recomputed", saved_y, MAX_ORDER)
saved_result.update({
    ...
    "source_max_order_reached": saved["max_order_reached"],
})
...
for i in range(N_RANDOM_SPARSE):
    ...
    result = iterate_y_direction("random_sparse_%02d" % (i + 1), y, MAX_ORDER)
    print_result(result)
...
for result in results:
    pr("%s: q=%s max_reached=%d" % (
        result["name"], result["quadratic_ok"], result["max_reached"]
    ))

maxed = [r for r in results if r["max_reached"] >= MAX_ORDER]
pr("")
pr("reached MAX_ORDER = %d / %d" % (len(maxed), len(results)))
pr("MAX_ORDER names = %s" % [r["name"] for r in maxed])

pr("")
pr("Conclusion")
pr("----------")
pr("This tests whether higher-order equations cut the 7-coordinate sparse slice.")
pr("If random sparse points reach MAX_ORDER, the whole sparse slice is a strong")
pr("formal-family candidate, not just the saved order-30 branch.")
