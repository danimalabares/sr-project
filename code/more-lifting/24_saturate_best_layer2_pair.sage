...
#
# Saturate the best sparse layer-2 correction from script 23.
#
# Script 23 found a much smaller correction:
#
#   h4 = m4 + t*x0*x1*x3*x7
#   h5 = m5 + t*x1*x3^2*x4
#
# with m1,m2,m3 raw.  It kills both named layer-3 membership tests with only
# eight first-colon witnesses.  Here we saturate that exact candidate.
#
# Run from:
#   code/more-lifting
#
...
OUT_FILE = "cache/24_saturate_best_layer2_pair.log"
...
def best_layer2_repairs(T):
    ...
    repairs = [
        ...
        monomial_T_from_exp(T, LAYER1_EXPS[1]),
        monomial_T_from_exp(T, LAYER1_EXPS[2]),
        monomial_T_from_exp(T, LAYER2_EXPS[0]) + T.gen(0) * q4,
        monomial_T_from_exp(T, LAYER2_EXPS[1]) + T.gen(0) * q5,
    ]
    return repairs, q4, q5


def saturate_best_layer2_pair(name, gen_vectors):
    pr(name)
    ...
    repairs, q4, q5 = best_layer2_repairs(T)

    pr("")
    pr("Best sparse layer-2 correction")
    pr("------------------------------")
    pr("q4 = %s" % q4)
    pr("q5 = %s" % q5)
    ...
    pr("")
    pr("Layer-3 hit check")
    pr("-----------------")
    pr("raw0 == I: %s" % raw0_eq_I)
    pr("t*u1 in J': %s" % (tu1_rem == 0))
    pr("t*u1 remainder: %s" % tu1_rem)
    pr("t*u2 in J': %s" % (tu2_rem == 0))
    pr("t*u2 remainder: %s" % tu2_rem)

    colon1 = Jprime.quotient(T.ideal(t))
    colon_eq = (colon1 == Jprime)
    pr("")
    pr("First colon")
    pr("-----------")
    pr("J' : t == J': %s" % colon_eq)
    try:
        layer1 = []
        for g in colon1.gens():
            r = T(g).reduce(gb_Jp)
            if r != 0:
                layer1.append(r)
        pr("new witnesses in J':t modulo J': %d" % len(layer1))
        for i, r in enumerate(layer1[:MAX_WITNESSES]):
            pr("  colon1[%d] = %s" % (i, r))
    except Exception as err:
        pr("first-colon witness reduction failed: %s" % err)

    Ksat = Jprime
    sat_steps = 0
    pr("")
    pr("Saturation layers")
    pr("-----------------")
    for step in range(1, MAX_SATURATION_STEPS + 1):
        Knext = Ksat.quotient(T.ideal(t))
        if Knext == Ksat:
        ...
        for g in Knext.gens():
            ...
            if r != 0:
                layer.append(r)
        pr("layer %d new witnesses = %d" % (step, len(layer)))
        for i, r in enumerate(layer[:MAX_WITNESSES]):
            pr("  layer%d[%d] = %s" % (step, i, r))
        Ksat = Knext
        sat_steps = step
    pr("saturation steps used = %d" % sat_steps)
    pr("K' : t == K': %s" % (Ksat.quotient(T.ideal(t)) == Ksat))

    K0 = specialize_t0_ideal(T, t, S, Ksat)
    K0_eq_I = ideal_eq(I_S, K0)
    pr("")
    pr("Saturated special fiber")
    pr("-----------------------")
    pr("(J':t^infinity)+(t) == I+(t): %s" % K0_eq_I)
    pr("I subset K0: %s" % all(K0.reduce(g) == 0 for g in I_S.gens()))
    pr("K0 subset I: %s" % all(I_S.reduce(g) == 0 for g in K0.gens()))
    try:
        pr("degree I = %s" % I_S.degree())
        pr("degree K0 = %s" % K0.degree())
        pr("hilbert I = %s" % I_S.hilbert_polynomial())
    ...
    pr("illegal special-fiber generators modulo I = %d" % len(extras))
    for i, r in enumerate(extras[:MAX_WITNESSES]):
        pr("  extra[%d] = %s" % (i, r))


pr("Saturating best sparse layer-2 pair")
pr("-----------------------------------")
...
saturate_best_layer2_pair("saved_order30_branch", saved["gen_vectors"])
