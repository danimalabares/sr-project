...
#
# Compare subansatze of the saved lifted-legal corrections.
#
# Script 21 showed that the full lifted hit still explodes under saturation.
# Here we test smaller deterministic subansatze to locate which lifted
# corrections trigger the first-colon explosion.
#
# Run from:
#   code/more-lifting
#
...
OUT_FILE = "cache/22_compare_lifted_legal_subansatz.log"
...
def compare_subansatze(name, gen_vectors):
    pr(name)
    ...
    pr("")
    pr("Saved q_i from script 20")
    pr("------------------------")
    for i, q in enumerate(q_polys, 1):
        pr("q_%d = %s" % (i, q))
    ...
    lifted_repairs = [
        make_repair(T, LAYER1_EXPS[0], q_polys[0]),
        make_repair(T, LAYER1_EXPS[1], q_polys[1]),
        make_repair(T, LAYER1_EXPS[2], q_polys[2]),
        make_repair(T, LAYER2_EXPS[0], q_polys[3]),
        make_repair(T, LAYER2_EXPS[1], q_polys[4]),
    ]

    ansatze = [
        ("A0_raw_all", raw_repairs),
        ("A1_lift_m1", [lifted_repairs[0]] + raw_repairs[1:]),
        ("A2_lift_m2", [raw_repairs[0], lifted_repairs[1]] + raw_repairs[2:]),
        ("A3_lift_m4", raw_repairs[:3] + [lifted_repairs[3], raw_repairs[4]]),
        ("A4_lift_m5", raw_repairs[:4] + [lifted_repairs[4]]),
        ("A5_lift_m1_m2", [lifted_repairs[0], lifted_repairs[1]] + raw_repairs[2:]),
        ("A6_lift_m4_m5", raw_repairs[:3] + [lifted_repairs[3], lifted_repairs[4]]),
        ("A7_lift_all", lifted_repairs),
    ]

    pr("")
    pr("First-colon comparison")
    pr("----------------------")
    rows = []
    for ansatz_name, ansatz_repairs in ansatze:
        pr("running %s" % ansatz_name)
        result = first_colon_diagnostic(T, t, S, J, I_S, ansatz_repairs, u1, u2)
        rows.append((ansatz_name, result))
        pr("  raw0==I %s | t*u1 in %s | t*u2 in %s | colon_eq %s | witnesses %d | legal %d | illegal %d" % (
            result["raw0_eq_I"], result["tu1_in"], result["tu2_in"],
            result["colon_eq"], result["n_witnesses"], result["n_legal"],
            result["n_illegal"],
        ))
    ...
    pr("Compact table")
    pr("-------------")
    pr("ansatz | raw0 | tu1 | tu2 | colon_eq | witnesses | legal | illegal")
    for ansatz_name, result in rows:
        pr("%s | %s | %s | %s | %s | %d | %d | %d" % (
            ansatz_name, result["raw0_eq_I"], result["tu1_in"],
            result["tu2_in"], result["colon_eq"], result["n_witnesses"],
            result["n_legal"], result["n_illegal"],
        ))


pr("Comparing lifted legal subansatze")
pr("---------------------------------")
...
compare_subansatze("saved_order30_branch", saved["gen_vectors"])
