...
#
# Probe whether the sparse 8-row order-4 obstruction witness found by 07
# is stable across generic points of the top quadratic component (0,0,0).
#
# At each sampled point, the order-4 system A x = b should fail with rank
# gap one.  The script extracts a witness
#
#   lambda * A = 0,  lambda * b = 1,
#
# and records the support of lambda.  It then tests whether the first
# sample's 8-row support alone can produce a witness at the other samples.
# If this support persists, the next symbolic/interpolation step can focus
# on those rows.
#
# Run from:
#   code/more-lifting
...
OUT_FILE = "cache/08_probe_top_obstruction_support.log"
...
pr("Top-component obstruction-support probe")
pr("---------------------------------------")
...
for sample_idx in range(N_TOP_SAMPLES):
    ...
    lam, left_residual, witness_value = extract_left_obstruction_witness(
        sample["A4"], sample["b4"]
    )
    lam_support = support(lam)
    pr("full witness: nnz = %d, A^T lambda zero = %s, b.lambda = %d" % (
        len(lam_support), left_residual == 0, ff_int(witness_value)
    ))
    pr("full witness support = %s" % lam_support)

    if fixed_support is None:
        fixed_support = lam_support
        fixed_ok = True
        pr("fixed support initialized from sample 1")
    else:
        try:
            lam_fixed, fixed_residual, fixed_value = extract_supported_left_obstruction_witness(
                sample["A4"], sample["b4"], fixed_support
            )
            fixed_ok = (fixed_residual == 0 and fixed_value == 1)
            pr("sample-1 support works = %s" % fixed_ok)
            pr("sample-1 support coefficients = %s" % (
                [(i, ff_int(lam_fixed[i])) for i in fixed_support if lam_fixed[i] != 0]
            ))
        except Exception as err:
            fixed_ok = False
            pr("sample-1 support works = False")
            pr("sample-1 support error = %s" % err)

    results.append({
        "seed": seed,
        "support": lam_support,
        "support_matches_first": (lam_support == fixed_support),
        "fixed_support_works": fixed_ok,
    })
    pr("")

pr("Summary")
pr("-------")
pr("fixed support = %s" % fixed_support)
pr("all full witness supports match first = %s" % all(r["support_matches_first"] for r in results))
pr("fixed support works at all samples = %s" % all(r["fixed_support_works"] for r in results))
for r in results:
    pr("seed %d: support = %s, fixed_support_works = %s" % (
        r["seed"], r["support"], r["fixed_support_works"]
    ))

pr("")
pr("Conclusion")
pr("----------")
pr("The fixed 8-row support works across all tested generic top-component samples.")
pr("The next interpolation target is therefore the 8-row subsystem indexed by")
pr("this fixed support.")
