# 15_test_colon_witness_repairs.sage
#
# Test direct repairs of the sparse-slice cubic family by adding stable
# t-torsion witnesses as extra family generators.
#
# Fixed rows:
#
#   [138, 139, 142, 143, 144, 146, 150, 157]
#
# Since the witnesses are in J after inverting t, adding them should not
# change the generic fiber.  Since their t=0 specializations lie in the SR
# ideal I, adding them also should not change the intended special fiber.
# This script checks whether such additions actually make t a nonzerodivisor
# without forcing the four recurring extra special-fiber equations.
#
# Run from:
#   code/more-lifting
#
...
OUT_FILE = "cache/15_test_colon_witness_repairs.log"
...
def build_family_ideal(gen_vectors, repair_exps=None):
    ...
    repair_exps = repair_exps or []
    repair_T = [monomial_T(e) for e in repair_exps]

    return T, t, S, T.ideal(F + repair_T), S.ideal(f_S), f_S
...
def flatness_diagnostic(name, gen_vectors, repair_name="original", repair_exps=None):
    pr(name)
    pr("  repair = %s" % repair_name)
    pr("  building family ideal")
    T, t, S, J, I_S, f_S = build_family_ideal(gen_vectors, repair_exps)
    pr("  generator nnz = %s" % {k: nnz(v) for k, v in gen_vectors.items()})
    pr("  repair generators = %d" % len(repair_exps or []))
    ...
    try:
        ...
        for g in colon.gens():
            ...
            if r != 0:
                witnesses.append(r)
        pr("  first-colon nonzero witnesses = %d" % len(witnesses))
        for i, r in enumerate(witnesses[:MAX_WITNESSES]):
            pr("    colon1[%d] = %s" % (i, r))
            pr("      t*colon1[%d] mod J = %s" % (i, T(t * r).reduce(gb_J)))
...
pr("Sparse-slice colon-witness repairs")
pr("-----------------------------------")
...
pr("MAX_WITNESSES = %d" % MAX_WITNESSES)
pr("REPAIRS = %s" % [name for name, _ in REPAIRS])
pr("")
...
for name, gen_vectors in results:
    for repair_name, repair_exps in REPAIRS:
        flat_results.append(((name, repair_name), flatness_diagnostic(name, gen_vectors, repair_name, repair_exps)))
        pr("")

pr("Conclusion")
pr("----------")
for (name, repair_name), result in flat_results:
    pr("%s / %s: J:(t)==J %s, K0==I %s, sat_steps %d" % (
        name, repair_name, result["colon_eq"], result["K0_eq_I"], result["sat_steps"]
    ))
