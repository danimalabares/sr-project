# 10_test_sparse_branch_wall.sage
#
# Evaluate the 09 determinant wall oracle on the saved sparse formal branch.
#
# Fixed rows:
#
#   [138, 139, 142, 143, 144, 146, 150, 157]
#
# The saved branch lifts formally to order 30.  Therefore, when we build its
# order-4 system using its saved lower-order corrections, the full system
# should be compatible.  This script checks what the generic top-chart wall
# determinant from 09 does on that branch:
#
#   det([A_FIXED_ROWS,PIVOT_COLUMNS | b_FIXED_ROWS]).
#
# A zero determinant, or a pivot-rank drop, means the branch lies on the wall
# or on the boundary of this determinant chart.
#
# Run from:
#   code/more-lifting
#
...
OUT_FILE = "cache/10_test_sparse_branch_wall.log"
...
pr("Sparse branch wall test")
pr("-----------------------")
pr("FIXED_ROWS = %s" % FIXED_ROWS)
pr("PIVOT_COLUMNS = %s" % PIVOT_COLUMNS)
pr("")

pr("Generic sanity check")
pr("--------------------")
generic = top_order4_system_for_seed(20260629)
generic_coeff_rank = generic["A4"].matrix_from_rows(FIXED_ROWS).matrix_from_columns(PIVOT_COLUMNS).rank()
generic_det, generic_augmented_rank = wall_determinant(
    generic["A4"], generic["b4"], FIXED_ROWS, PIVOT_COLUMNS
)
pr("generic order 4 lifts = %s" % generic["lifts4"])
pr("generic full rank_A/rank_B = %d / %d" % (generic["rank_A4"], generic["rank_B4"]))
pr("generic pivot coefficient rank = %d" % generic_coeff_rank)
pr("generic wall determinant = %d" % ff_int(generic_det))
pr("generic wall determinant nonzero = %s" % (generic_det != 0))
pr("generic 8x8 augmented rank = %d" % generic_augmented_rank)
pr("")
...
saved_row_rank = saved_A4.matrix_from_rows(FIXED_ROWS).rank()
saved_coeff_rank = saved_A4.matrix_from_rows(FIXED_ROWS).matrix_from_columns(PIVOT_COLUMNS).rank()
saved_det, saved_augmented_rank = wall_determinant(
    saved_A4, saved_b4, FIXED_ROWS, PIVOT_COLUMNS
)
pr("saved order 4 lifts = %s" % saved_lifts4)
pr("saved full rank_A/rank_B = %d / %d" % (saved_rank_A4, saved_rank_B4))
pr("saved fixed-row coefficient rank = %d" % saved_row_rank)
pr("saved pivot coefficient rank = %d" % saved_coeff_rank)
pr("saved wall determinant = %d" % ff_int(saved_det))
pr("saved wall determinant zero = %s" % (saved_det == 0))
pr("saved 8x8 augmented rank = %d" % saved_augmented_rank)

pr("")
pr("Conclusion")
pr("----------")
if saved_det == 0:
    pr("The saved sparse branch lies on the determinant wall in this chart.")
elif saved_coeff_rank < len(PIVOT_COLUMNS):
    pr("The saved sparse branch lies on the boundary of the determinant chart.")
else:
    pr("The saved sparse branch does not vanish on this determinant oracle;")
    pr("the wall chart or saved-correction comparison needs refinement.")
