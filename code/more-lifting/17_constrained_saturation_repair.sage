# 17_constrained_saturation_repair.sage
#
# Try to repair the sparse-slice cubic family by adding only legal elements
# from the t-saturation.  "Legal" means the element has t=0 specialization in
# the SR(M) ideal, so adjoining it cannot change the intended special fiber.
#
# Run from:
#   code/more-lifting
#
...
OUT_FILE = "cache/17_constrained_saturation_repair.log"
...
pr("Sparse-slice constrained saturation repair")
pr("------------------------------------------")
...
pr("MAX_LEGAL_CLOSURE_STEPS = %d" % MAX_LEGAL_CLOSURE_STEPS)
pr("")
...
for name, gen_vectors in results:
    flat_results.append((name, constrained_saturation_repair(name, gen_vectors)))
    pr("")

pr("Conclusion")
pr("----------")
for name, result in flat_results:
    pr("%s: J:(t)==J %s, K0==I %s, sat_steps %d, legal_added %d, illegal_blockers %d" % (
        name, result["colon_eq"], result["K0_eq_I"], result["sat_steps"],
        result["n_added"], result["n_illegal_blockers"]
    ))
