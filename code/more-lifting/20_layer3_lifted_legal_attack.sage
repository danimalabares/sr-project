# 20_layer3_lifted_legal_attack.sage
#
# Focused attack on the first illegal torsion layer.
#
# We start from the sparse cubic family J and add lifted legal witnesses
#
#   h_i = m_i + t*q_i.
#
# The constant terms m_i are the legal layer-1/layer-2 torsion witnesses, so
# adding them should not change the raw special fiber.  The test here is only
# whether random q_i in a small finite degree-4 monomial ansatz can make
# t*u1 or t*u2 stop being consequences of the enlarged ideal.  We do not test
# full flatness in this script.
#
# Run from:
#   code/more-lifting
#
...
OUT_FILE = "cache/20_layer3_lifted_legal_attack.log"
...
def layer3_lifted_legal_attack(name, gen_vectors, rng):
    pr(name)
    ...
    Jraw = T.ideal(list(J.gens()) + raw_legal_repair_polys(T))
    raw0 = specialize_t0_ideal(T, t, S, Jraw)
    raw0_eq_I = ideal_eq(I_S, raw0)
    gb_raw = Jraw.groebner_basis()
    raw_tu1_in = (T(t * u1).reduce(gb_raw) == 0)
    raw_tu2_in = (T(t * u2).reduce(gb_raw) == 0)
    pr("  raw legal baseline: raw0==I %s, t*u1 in J_raw %s, t*u2 in J_raw %s" % (
        raw0_eq_I, raw_tu1_in, raw_tu2_in
    ))

    hits = []
    best = []
    for trial in range(1, N_RANDOM_REPAIRS + 1):
        repairs, supports = lifted_repair_polys(T, rng)
        Jtrial = T.ideal(list(J.gens()) + repairs)
        raw0 = specialize_t0_ideal(T, t, S, Jtrial)
        raw0_eq_I = ideal_eq(I_S, raw0)

        gb = Jtrial.groebner_basis()
        tu1_rem = T(t * u1).reduce(gb)
        tu2_rem = T(t * u2).reduce(gb)
        tu1_in = (tu1_rem == 0)
        tu2_in = (tu2_rem == 0)

        score = (
            0 if raw0_eq_I else 1,
            int(tu1_in) + int(tu2_in),
        )
        best.append((score, trial, raw0_eq_I, tu1_in, tu2_in, tu1_rem, tu2_rem, supports))

        if raw0_eq_I and ((not tu1_in) or (not tu2_in)):
            hits.append((trial, tu1_in, tu2_in, tu1_rem, tu2_rem, supports))
            pr("  HIT trial %d: raw0==I %s, t*u1 in J' %s, t*u2 in J' %s" % (
                trial, raw0_eq_I, tu1_in, tu2_in
            ))
            if not tu1_in:
                pr("    t*u1 remainder = %s" % tu1_rem)
            if not tu2_in:
                pr("    t*u2 remainder = %s" % tu2_rem)
            break
        if trial <= 5 or trial == N_RANDOM_REPAIRS:
            pr("  trial %02d: raw0==I %s, t*u1 in J' %s, t*u2 in J' %s" % (
                trial, raw0_eq_I, tu1_in, tu2_in
            ))
    ...
    pr("  hits = %d" % len(hits))
    pr("  best trials:")
    for score, trial, raw0_eq_I, tu1_in, tu2_in, tu1_rem, tu2_rem, supports in best[:5]:
        pr("    trial %02d score %s raw0==I %s t*u1 in J' %s t*u2 in J' %s" % (
            trial, score, raw0_eq_I, tu1_in, tu2_in
        ))
        if not tu1_in:
            pr("      t*u1 remainder = %s" % tu1_rem)
        if not tu2_in:
            pr("      t*u2 remainder = %s" % tu2_rem)
        pr("      q-support sizes = %s" % [len(s) for s in supports])
...
results.append(("saved_order30_branch", saved["gen_vectors"]))

rng = random.Random(int(SEED_BASE))

pr("Preparing random sparse-support directions")
pr("------------------------------------------")
for i in range(N_RANDOM_SPARSE):
    y = random_y_on_support(rng, SPARSE_SUPPORT)
    state, infos = state_for_y(y, MAX_ORDER)
    name = "random_sparse_%02d" % (i + 1)
    if state is None:
        pr("%s did not reach order %d; infos = %s" % (name, MAX_ORDER, infos))
        continue
    pr("%s reached order %d; generator nnz = %s" % (
        name, MAX_ORDER, {k: nnz(v) for k, v in state["gen_vectors"].items()}
    ))
    results.append((name, state["gen_vectors"]))
pr("")

pr("Layer-3 membership diagnostics")
pr("------------------------------")
flat_results = []
repair_rng = random.Random(builtins.int(20261620))
for name, gen_vectors in results:
    flat_results.append((name, layer3_lifted_legal_attack(name, gen_vectors, repair_rng)))
    pr("")

pr("Conclusion")
pr("----------")
for name, result in flat_results:
    pr("%s: hits %d" % (name, len(result["hits"])))
