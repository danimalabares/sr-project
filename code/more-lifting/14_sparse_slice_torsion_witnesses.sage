...
#
# Inspect t-torsion witnesses for cubic families coming from the 7-coordinate
# sparse slice.
#
# Fixed rows:
#
...
#
# The previous flatness samples showed that J has t-torsion and that
# saturation adds the same four extra special-fiber equations.  This script
# records the first colon/saturation witnesses so we can see what missing
# family-ideal structure is responsible.
#
# Run from:
#   code/more-lifting
#
...
OUT_FILE = "cache/14_sparse_slice_torsion_witnesses.log"
...
pr("Sparse-slice torsion witnesses")
pr("------------------------------")
...
for name, gen_vectors in results:
    flat_results.append((name, flatness_diagnostic(name, gen_vectors)))
    pr("")

pr("Conclusion")
pr("----------")
for name, result in flat_results:
    pr("%s: J:(t)==J %s, K0==I %s, sat_steps %d" % (
        name, result["colon_eq"], result["K0_eq_I"], result["sat_steps"]
    ))
