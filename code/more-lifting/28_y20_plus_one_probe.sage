# 28_y20_plus_one_probe.sage
#
# Keep y20 nonzero and add one more T^1 coordinate at a time.
#
# Scripts 26 and 27 showed that adding a nonzero y20 coefficient to the sparse
# branch structurally drops the visible saturated-special-fiber defect from four
# extra generators to three.  This script fixes y20=37 and searches for a
# second structural drop by adding one further T^1 coordinate yj with a small
# coefficient grid.  The full grid is used for the quadratic filter; the
# expensive lifting/saturation stage first tests one representative coefficient
# per surviving coordinate.
#
# Run from:
#   code/more-lifting
#
...
OUT_FILE = "cache/28_y20_plus_one_probe.log"
...
BASE_Y20 = 37
ADD_COEFFS = [1, 2, 3, 5, 7, 11, 32002]
PROMISING_EXTRA_THRESHOLD = 2
PROMISING_MAX_ORDER = 6
MAX_LIFT_COEFFS_PER_COORD = 1
SEED_BASE = 20260629
...
def flatness_diagnostic(name, gen_vectors):
    pr(name)
    ...
    I_subset_K0 = all(K0.reduce(g) == 0 for g in I_S.gens())
    K0_subset_I = all(I_S.reduce(g) == 0 for g in K0.gens())
    pr("  I subset K0 = %s" % I_subset_K0)
    pr("  K0 subset I = %s" % K0_subset_I)
    pr("  K0 == I = %s" % (I_subset_K0 and K0_subset_I))
    ...
    try:
        ...
        for g in K0.gens():
            ...
            if r != 0:
                extras.append(r)
        first_extras = extras[:6]
        pr("  extra K0 generators modulo I = %d" % len(extras))
        pr("  first extras = %s" % first_extras)
    except Exception as err:
    ...
    return {
        "colon_eq": colon_eq,
        "sat_steps": sat_steps,
        "K0_eq_I": I_subset_K0 and K0_subset_I,
        "t_saturated": Ksat.quotient(T.ideal(t)) == Ksat,
        "extra_count": None if extras is None else len(extras),
        "first_extras": first_extras,
    }


def y20_line_point(y20_coeff):
    y = [K(0)] * 53
    ...
    v1 = K(0)
    v2 = K(0)
    put_values(y, [0, 3, 7], [a * u0, a * u1, a * u2])
    put_values(y, [31, 35, 36], [b * u0, b * u1, b * u2])
    put_values(y, [9, 48], [a * v0, a * v1])
    put_values(y, [23, 39], [b * v0, b * v1])
    put_values(y, [52, 44], [a * v2, b * v2])
    ...
    w5 = K(0)
    put_values(y, [1, 4, 22, 26, 27, 29],
               [c * w0, c * w1, c * w2, c * w3, c * w4, c * w5])
    put_values(y, [19, 20, 47, 38, 43, 45],
               [d * w0, d * w1, d * w2, d * w3, d * w4, d * w5])
    ...
    q0 = K(0)
    q1 = K(89)
    put_values(y, [6, 8, 15, 16], [e * p0, e * p1, e * p2, e * p3])
    put_values(y, [40, 37, 49, 50], [f * p0, f * p1, f * p2, f * p3])
    put_values(y, [32, 33], [e * q0, e * q1])
    put_values(y, [24, 25], [f * q0, f * q1])

    put_values(y, [2, 34, 41], [K(97), K(101), K(103)])

    if not is_quadratic_solution(y):
        bad = [i for i, value in enumerate(eval_quadrics(y)) if value != 0]
        raise RuntimeError("y20-line candidate failed quadratic equations: %s" % bad)
    return y


Y20_SUPPORT = sorted(SPARSE_SUPPORT + [20])


def y20_plus_one_point(extra_index, extra_coeff):
    y = y20_line_point(BASE_Y20)
    y[extra_index] = K(extra_coeff)
    return y


def format_extra(poly):
    return str(poly)


pr("y20 plus one-coordinate probe")
pr("-----------------------------")
pr("SPARSE_SUPPORT = %s" % SPARSE_SUPPORT)
pr("Y20_SUPPORT = %s" % Y20_SUPPORT)
pr("BASE_Y20 = %s" % BASE_Y20)
pr("ADD_COEFFS = %s" % ADD_COEFFS)
pr("MAX_ORDER = %d" % MAX_ORDER)
pr("MAX_SATURATION_STEPS = %d" % MAX_SATURATION_STEPS)
pr("PROMISING_EXTRA_THRESHOLD = %d" % PROMISING_EXTRA_THRESHOLD)
pr("PROMISING_MAX_ORDER = %d" % PROMISING_MAX_ORDER)
pr("MAX_LIFT_COEFFS_PER_COORD = %d" % MAX_LIFT_COEFFS_PER_COORD)
pr("")
...
for j in range(53):
    if j in Y20_SUPPORT:
        continue
    for coeff in ADD_COEFFS:
        y = y20_plus_one_point(j, coeff)
        q_ok = is_quadratic_solution(y)
        tested.append((j, coeff, q_ok))
        if q_ok:
            quadratic_candidates.append((j, coeff, y))
            pr("quadratic survivor: add y%d = %s, support = %s" % (
                j, K(coeff), support(y)
            ))

pr("")
pr("quadratic survivors = %d / %d" % (len(quadratic_candidates), len(tested)))
pr("")

quadratic_by_coord = defaultdict(list)
for j, coeff, y in quadratic_candidates:
    quadratic_by_coord[j].append((j, coeff, y))

lift_candidates = []
for j in sorted(quadratic_by_coord):
    lift_candidates.extend(quadratic_by_coord[j][:MAX_LIFT_COEFFS_PER_COORD])

pr("coordinates surviving quadratic filter = %d" % len(quadratic_by_coord))
pr("order-4 representatives selected = %d" % len(lift_candidates))
for j in sorted(quadratic_by_coord):
    coeffs = [int(c) for _, c, _ in quadratic_by_coord[j]]
    chosen = [int(c) for _, c, _ in quadratic_by_coord[j][:MAX_LIFT_COEFFS_PER_COORD]]
    pr("  y%d: quadratic coeffs=%s, lifted reps=%s" % (j, coeffs, chosen))
pr("")

pr("Order-4 lifting tests")
pr("---------------------")
for j, coeff, y in lift_candidates:
    name = "y20_37_plus_y%d_%s" % (j, coeff)
    state, infos = state_for_y(y, MAX_ORDER)
    max_reached = 1
    for order in sorted(infos):
        if infos[order]["success"]:
            max_reached = order
        else:
            break
    row = {
        ...
        "extra_coeff": K(coeff),
        "support": support(y),
        "support_size": len(support(y)),
        "max_reached": max_reached,
        "infos": infos,
        "state": state,
    }
    pr("%s" % name)
    pr("  added y%d = %s" % (j, K(coeff)))
    pr("  support(%d) = %s" % (row["support_size"], row["support"]))
    for order in sorted(infos):
        info = infos[order]
        pr("  order %d: lifts=%s rank_A=%d rank_B=%d equations=%s" % (
            order, info["success"], info["rank_A"], info["rank_B"],
    ...
    if state is not None:
        pr("  reached MAX_ORDER; generator nnz = %s" % {
            k: nnz(v) for k, v in state["gen_vectors"].items()
        })
        survivors.append(row)
    pr("")

survivors.sort(key=lambda r: (r["support_size"], r["name"]))

pr("Order-4 survivors")
pr("-----------------")
pr("reached MAX_ORDER = %d / %d lifted representatives" % (
    len(survivors), len(lift_candidates)
))
for row in survivors:
    pr("%s add y%d=%s support_size=%d support=%s" % (
        row["name"], row["extra_index"], row["extra_coeff"],
        row["support_size"], row["support"]
    ))
pr("")

pr("Flatness diagnostics")
pr("--------------------")
flat_results = []
for row in survivors:
    result = flatness_diagnostic(row["name"], row["state"]["gen_vectors"])
    flat_results.append((row, result))
    pr("")

flat_results.sort(key=lambda item: (
    999 if item[1]["extra_count"] is None else item[1]["extra_count"],
    item[1]["sat_steps"],
    item[0]["extra_index"],
    int(item[0]["extra_coeff"]),
))

pr("Ranked candidates")
pr("-----------------")
for row, result in flat_results:
    pr("%s: add y%d=%s, support_size %d, extras %s, J:(t)==J %s, K0==I %s, sat_steps %d, saturated %s" % (
        row["name"], row["extra_index"], row["extra_coeff"],
        row["support_size"], result["extra_count"], result["colon_eq"],
        result["K0_eq_I"], result["sat_steps"], result["t_saturated"]
    ))
    if result["first_extras"]:
        pr("  first extras = %s" % [format_extra(g) for g in result["first_extras"][:3]])

promising = [
    (row, result)
    for row, result in flat_results
    if result["extra_count"] is not None
    and result["extra_count"] <= PROMISING_EXTRA_THRESHOLD
]
...
pr("----------------")
if promising:
    for row, result in promising:
        y = y20_plus_one_point(row["extra_index"], row["extra_coeff"])
        ...
        for order in sorted(infos):
            if infos[order]["success"]:
                ...
                break
        pr("%s: extras %s, rerun max_reached %d" % (
            row["name"], result["extra_count"], max_reached
        ))
        for order in sorted(infos):
            info = infos[order]
            pr("  order %d: lifts=%s rank_A=%d rank_B=%d equations=%s" % (
        ...
        if state is not None:
            pr("  rerun generator nnz = %s" % {
                k: nnz(v) for k, v in state["gen_vectors"].items()
            })
            pr("  rerun flatness diagnostic")
            flatness_diagnostic(row["name"] + "_order%d" % PROMISING_MAX_ORDER,
                                state["gen_vectors"])
else:
    pr("No candidate had %d or fewer extras." % PROMISING_EXTRA_THRESHOLD)

pr("")
pr("Conclusion")
pr("----------")
if flat_results:
    best_count = flat_results[0][1]["extra_count"]
    pr("best extra-generator count = %s" % best_count)
    if best_count is not None and best_count < 3:
        pr("Some y20-plus-one candidates improve below the y20-line count of 3.")
    else:
        pr("No tested y20-plus-one candidate improves below the y20-line count of 3.")
else:
    pr("No y20-plus-one candidate reached order 4.")
