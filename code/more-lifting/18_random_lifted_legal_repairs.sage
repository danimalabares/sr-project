# 18_random_lifted_legal_repairs.sage
#
# Randomly test lifted legal repair generators.  Each added generator has
# constant term in the SR(M) ideal, so the raw t=0 ideal remains SR(M).  The
# t-parts are allowed to involve the layer-3 monomials that appeared as
# illegal saturation blockers, in the hope of cancelling the colon chain
# before saturation forces them into the special fiber.
#
# Run from:
#   code/more-lifting
#
...
OUT_FILE = "cache/18_random_lifted_legal_repairs.log"
...
pr("Sparse-slice random lifted legal repairs")
pr("----------------------------------------")
...
for name, gen_vectors in results:
    flat_results.append((name, random_lifted_repair_search(name, gen_vectors, repair_rng)))
    pr("")
