...
def test_lift_to_order_2(direction, test_number):
    ...
    for r in range(syz.nrows()):
        ...
        for e, coeffs in rows_by_exp.items():
            ...
            if add_row(entries, rhs, coeffs, const):
                n_order1_rows += 1
        ...
        for e, coeffs in rows_by_exp.items():
            if add_row(entries, rhs, coeffs, BASE_FIELD(0)):
                n_order2_rows += 1
    ...
    A1 = A.matrix_from_rows(range(n_order1_rows))
    b1 = vector(BASE_FIELD, list(b)[:n_order1_rows])
    B1 = A1.augment(matrix(BASE_FIELD, n_order1_rows, 1, list(b1), sparse=True))
    ...
    print("  order-1 equations =", n_order1_rows)
    print("  order-2 equations =", n_order2_rows)
    print("order-1 lift exists:", order1_ok, " ranks", rank_A1, rank_B1)
    print("order-2 lift exists:", lifts, " ranks", rank_A, rank_B)
    print()
