# 17_replay_cubic_family_second_prime.sage
#
# Replay the exported cubic family over a second prime.
#
# Run from:
...
#
# Then run:
#   sage 17_replay_cubic_family_second_prime.sage
#
# Input:
#   ../part-1.pkl
#   cache/cubic_family_data.json
#
# The script imports the sparse integer coefficient data for
# G_1,G_2,G_3,A_1, reduces it modulo REPLAY_PRIME, and checks
# the same four cubic-family identities modulo I and exactly.
...
OUTPUT_FILE = "cache/cubic_family_second_prime_verification.sobj"
...
# ------------------------------------------------------------
# Import cubic data over replay prime
# ------------------------------------------------------------

G = {}
G_terms = {}
for k in [1, 2, 3]:
    G[k] = sparse_data_to_vector(exported["G"][str(k)], n_params)
    G_terms[k] = direction_to_terms(G[k])

A1 = sparse_data_to_vector(exported["A"]["1"], n_corr)
A1_terms = syz_vector_to_terms(A1)

print("Imported cubic data")
print("-------------------")
for k in [1, 2, 3]:
...
print("Second-prime cubic-family identity checks")
print("-----------------------------------------")
...
if not all_modI_ok:
    raise RuntimeError("second-prime replay failed modulo I")
