# 09_test_order5_from_saved_lift.sage
#
# Test whether the saved order-4 lift extends to order 5.
#
# Run from:
...
#   = 0 in S/I.
#
# Here G_1=phi, G_2=psi, G_3=chi, G_4=eta, ...
# and A_1=alpha, A_2=beta, A_3=gamma, ...
#
# Unknown terms:
#   s*G_n + A_{n-1}*phi
#
...
INPUT_FILE = "cache/one_order4_lift.sobj"
OUTPUT_FILE = "cache/one_order5_lift.sobj"

TARGET_ORDER = 5

# known generator corrections G_k, k=1,...,TARGET_ORDER-1
GEN_SOLUTION_NAMES = {1: "direction", 2: "psi_solution", 3: "chi_solution", 4: "eta_solution"}

# known syzygy corrections A_a, a=1,...,TARGET_ORDER-2
SYZ_SOLUTION_NAMES = {1: "alpha_solution", 2: "beta_solution", 3: "gamma_solution"}

NEW_GEN_SOLUTION_NAME = "theta_solution"
NEW_SYZ_SOLUTION_NAME = "delta_solution"
...
# ------------------------------------------------------------
# Syzygy preprocessing, exactly as in 05/06/07
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Reconstruct syzygy-correction columns exactly as before
# ------------------------------------------------------------

# All syzygy corrections A_a use the same indexing as alpha_col in 06/07.
# Columns in each solve are:
#   0, ..., n_params-1      new generator correction columns
#   n_params, ...           new syzygy correction columns
...
# ------------------------------------------------------------
# Load saved previous-order lift
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Build target-order linear system
# ------------------------------------------------------------

# Unknowns:
#   G_TARGET_ORDER: columns 0, ..., n_params-1
#   A_{TARGET_ORDER-1}: columns n_params, ..., n_params+n_corr-1
...
for r in range(syz.nrows()):
    ...
    #   sum_j A_{TARGET_ORDER-1,rj} * phi_j
    # --------------------------------------------------------
    phi = gen_terms[1]
    ...
    # --------------------------------------------------------
    # Constant terms:
    #   sum_{a=1}^{TARGET_ORDER-2} A_a * G_{TARGET_ORDER-a}
    # --------------------------------------------------------
    for a in range(1, TARGET_ORDER - 1):
        G_index = TARGET_ORDER - a
        A_index = a
...
print("Order-%d linear system" % TARGET_ORDER)
print("---------------------")
...
print("  order-%d equations =" % TARGET_ORDER, len(target_rows))
print()
...
print("Order-%d rank test" % TARGET_ORDER)
print("-----------------")
print("rank_A =", rank_A)
print("rank_augmented =", rank_augmented)
print("lifts to order %d:" % TARGET_ORDER, lifts)
print()

if lifts:
    sol = A.solve_right(b)
    new_gen_solution = sol[:n_params]
    new_syz_solution = sol[n_params:]

    out = {}

    for k in sorted(GEN_SOLUTION_NAMES):
        out[GEN_SOLUTION_NAMES[k]] = gen_vectors[k]
    for a in sorted(SYZ_SOLUTION_NAMES):
        out[SYZ_SOLUTION_NAMES[a]] = syz_vectors[a]

    out[NEW_GEN_SOLUTION_NAME] = new_gen_solution
    out[NEW_SYZ_SOLUTION_NAME] = new_syz_solution
    out["rank_A"] = rank_A
    out["rank_augmented"] = rank_augmented
    out["rank_B"] = rank_augmented

    save(out, OUTPUT_FILE)
    print("Saved", OUTPUT_FILE)
else:
    print("No order-%d lift found for this saved order-%d lift." % (TARGET_ORDER, TARGET_ORDER - 1))
